<x-dashboard-layout>
  ...
  <div class="header-section">
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px;">
      <div>
        ...
        <p class="welcome-text">Manage PSG officer role applications from students</p>
  ...
  <div class="header-section">
      <div class="bg-white shadow-sm sm:rounded-lg">
        <h3 class="text-lg font-semibold mb-4">PSG Role Applications</h3>
              ...
              <tr style="border-bottom: 1px solid #f3f4f6; transition: background-color 0.2s ease;" onmouseover="this.style.backgroundColor='#f9fafb'" onmouseout="this.style.backgroundColor='transparent'">
                ...
                <td style="padding: 16px; text-align: center;">
                  <div style="display: flex; gap: 8px; justify-content: center; flex-wrap: wrap;">
                    @if($application->status == 'pending')
                      <!-- Approve Button -->
                      <form action="{{ route('admin.approvepsg', $application->id) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('PATCH')
                      ...
                      <!-- Reject Button -->
                      <form action="{{ route('admin.rejectpsg', $application->id) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
                    ...
                    @elseif($application->status == 'approved')
                      <!-- Sync Role Button (re-applies PSG officer role if out of sync) -->
                      <form action="{{ route('admin.approvepsg', $application->id) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('PATCH')
  ...
  <script>
    ...
    function showRevokeConfirmation(applicationId, fullname) {
      showConfirmationModal(
        ...
        `Are you sure you want to revoke PSG officer privileges for ${fullname}? This will remove their officer role and restore their student account.`,
        'Revoke Access',
        '#f59e0b',
        '#f59e0b',
        '<path stroke-linecap="round" stroke-linejoin="round" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728L5.636 5.636m12.728 12.728L18.364 5.636M5.636 18.364l12.728-12.728" />',
        '{{ route("admin.revokepsg", "__ID__") }}',
        applicationId
      );
    }
    ...
    function showReconsiderConfirmation(applicationId, fullname) {
      showConfirmationModal(
        ...
        '#8b5cf6',
        '<path stroke-linecap="round" stroke-linejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />',
        '{{ route("admin.reconsiderpsg", "__ID__") }}',
        applicationId
      );
    }
