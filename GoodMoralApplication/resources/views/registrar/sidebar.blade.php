...
<aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-64'"
  ...
  <nav>
    <!-- Dashboard -->
    <!-- Application -->
    <a href="{{ route('registrar.goodMoralApplication') }}"
      class="gap-2 items-center flex h-20 px-4 py-2 {{ request()->routeIs('registrar.goodMoralApplication') ? 'bg-gray-800 text-spupGold' : 'text-white hover:bg-gray-800 hover:text-spupGold' }}" style="position: relative;">
      ...
      Dashboard
      <span class="notification-bell" id="registrar-applications-bell" style="display: none; margin-left: auto; background: rgba(255, 193, 7, 0.1); padding: 2px 6px; border-radius: 12px; font-size: 11px; font-weight: 600; align-items: center; gap: 4px;">
        ...
        <span class="notification-count" id="registrar-applications-count" style="color: #ffc107; min-width: 16px; text-align: center;">0</span>
      </span>
    </a>
...
function updateRegistrarNotificationCounts() {
  fetch('/registrar/notification-counts')
    .then(response => response.json())
    .then(data => {
      console.log('Registrar notification data received:', data);

      // Update application notifications
      updateNotificationBell('registrar-applications', data.applicationNotifications || 0);
    })
    .catch(error => {
      console.log('Error fetching Registrar notification counts:', error);
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
  console.log('Registrar sidebar loaded, initializing notifications...');
