<x-dashboard-layout>
    <x-slot name="roleTitle">Head OSA</x-slot>

    <x-slot name="navigation">
        @include('head_osa.sidebar')
    </x-slot>

    <!-- Header Section -->
    <div class="header-section">
        <div>
            <h1 class="role-title">Head OSA Profile</h1>
                    ...
                    <form method="post" action="{{ route('profile.update') }}" style="display: grid; gap: 20px;">
                        ...
                        <div style="display: flex; align-items: center; gap: 16px;">
                            ...
                            @if (session('status') === 'profile-updated')
                                <p style="color: #28a745; font-size: 14px; margin: 0;">{{ __('Saved.') }}</p>
                            @endif
                        </div>
                    </form>
                </section>
        ...
        <div style="background: white; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); overflow: hidden;">
            ...
            <div style="padding: 24px;">
                <section>
                    ...
                    <form method="post" action="{{ route('profile.update-email') }}" style="display: grid; gap: 20px;">
                        @csrf
                        ...
                        <div>
                            <label for="current_password_email" style="display: block; margin-bottom: 8px; font-weight: 500; color: #495057;">{{ __('Current Password') }}</label>
                        ...
                        <div style="display: flex; align-items: center; gap: 16px;">
                            <button type="submit" class="btn-primary">{{ __('Send Verification Email') }}</button>

                            @if (session('status') === 'email-verification-sent')
                                <p style="color: #28a745; font-size: 14px; margin: 0;">{{ __('Verification email sent.') }}</p>
                            @endif
                        </div>
                    </form>
                </section>
            ...
            <div style="padding: 24px;">
                <section>
                    <form method="post" action="{{ route('password.update') }}" style="display: grid; gap: 20px;">
                        ...
                        <div>
                            <label for="update_password_current_password" style="display: block; margin-bottom: 8px; font-weight: 500; color: #495057;">{{ __('Current Password') }}</label>
                        ...
                        <div>
                            <label for="update_password_password" style="display: block; margin-bottom: 8px; font-weight: 500; color: #495057;">{{ __('New Password') }}</label>
                        ...
                        <div>
                            <label for="update_password_password_confirmation" style="display: block; margin-bottom: 8px; font-weight: 500; color: #495057;">{{ __('Confirm Password') }}</label>
